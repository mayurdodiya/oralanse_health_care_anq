...
exports.getProductCategoryListingSeraching = async (req, res) => {
  try {
    ...
    const productData = await ecommerceService.getProductCategoryList({ page, size, s, categoryType: options.PortalType.DOCTOR }, res)
...
exports.viewProductListByCategory = async (req, res) => {
  try {
    ...
    if (!categoryId) {
      return res.status(200).json({ success: "false", message: message.NO_DATA("This category") })
    }
...
exports.verifyOrder = async (req, res) => {
  ...
  if (razorpay_signature === generated_signature) {
    ...
    // send to user ------
    const context = await emailTmplateServices.getOrderGenerateContext({ /* fadd data as per context need */ })
...
exports.addShippingAddress = async (req, res) => {
  try {
    ...
    res.status(200).json({ message: message.ADD_DATA("Order address") })
  } catch (error) {
    res.status(200).json({ success: "false", message: error.message })
  }
};
...
exports.orderListing = async (req, res) => {
  try {
    ...
    const data = await commonServices.getAndCountAll(Orders, {
      where: { user_id: userId }, attributes: ['id', 'user_id', 'order_number', 'createdAt'],
      include: [
        {
    ...
    // console.log(modifyOrderRes);
    res.status(200).json({ success: "true", message: message.GET_DATA("Order"), data: responseData });
  } catch (error) {
    console.log(error);
    res.status(200).json({ success: "false", message: error.message })
  }
...
exports.shippingAddressListing = async (req, res) => {
  try {
    ...
    res.status(200).json({ success: "true", message: message.GET_DATA("Address"), data: data });
  } catch (error) {
    res.status(200).json({ success: "false", message: error.message })
  }
};
