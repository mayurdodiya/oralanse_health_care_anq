...
module.exports = {
  getAllFacialAstheticClinics: async (data) => {
    let arr1 = []
    var clinicTreatment = await commonServices.getAll(ClinicTreatment, { attributes: ["id", "clinic_id"], include: [{ model: Treatment, as: "treatments", attributes: ["id", "name"], where: { name: data.treatment } }] })
    clinicTreatment.length != 0 && clinicTreatment.map(i => { arr1.push(i.clinic_id) })
    let arr2 = []
